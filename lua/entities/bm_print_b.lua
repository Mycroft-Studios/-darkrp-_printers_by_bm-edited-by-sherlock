...
function ENT:Initialize()
	...
	if ( SERVER ) then
		...
		self:SetNWFloat("proc_power", 5)
		self:SetNWFloat("proc_paper", 5)
		self:SetNWFloat("proc_cooling", 5)
		self:SetNWInt("money", 0)

		function ENT:Think()
		if (self:GetNWFloat("proc_cooling" ) > 110) then
		 self:SetNWFloat("proc_cooling", 110)
		end
...
if(self:GetNWEntity("scr") != NULL)then
	...
	cam.Start3D2D( self:GetNWEntity("scr"):GetPos() , self:GetNWEntity("scr"):GetAngles() + Angle(0,0,0), 0.1 )
		...
		surface.SetDrawColor( 255, 255, 0, 255 )
		if(self:GetNWInt("proc_cooling", 1) > 0)then
