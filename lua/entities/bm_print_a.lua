...
function ENT:Initialize()
	...
	if ( SERVER ) then
		...
		local payday = tonumber(file.Read( "Printer_A.txt", "DATA" ))
		if(payday == nil)then
		payday = 30
		else
...
if(self:GetNWEntity("scr") != NULL)then
	...
	cam.Start3D2D( self:GetNWEntity("scr"):GetPos() , self:GetNWEntity("scr"):GetAngles() + Angle(0,0,0), 0.1 )
		...
		surface.SetFont( "printfont" )
		if(self:GetNWFloat("proc_paper", 0) >= 10)then
		surface.SetTextColor( 255, 255, 255, 255 )
		else
		surface.SetTextColor( 255, 0, 0, 255 )
		end
...
function ENT:Think()
	...
	local addmoney = self:GetNWInt("money", 0) + self:GetNWInt("payday", 30)
